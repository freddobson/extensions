...
$ExportFile = "$env:temp\CachedKerberosTickets.txt"
...
$ExportFile2 = "$env:temp\kerbsessions.csv"
foreach ($Session in $sessions) {
    if ($n = 0) {
        $sessioninfo.$session | Export-Csv -NoTypeInformation -Encoding 'UTF8' -Append -Path $ExportFile2
    } else {
        $sessioninfo.$session | Export-Csv -NoTypeInformation -Encoding 'UTF8' -Append -Path $ExportFile2
    }
}
...
tempfile = os.getenv("TEMP") .. "\\kerbsessions.csv"
file = io.open(tempfile, "r") -- r read mode
if file then
    file:close()
    for line in io.lines(tempfile) do
        if line ~= nil then
            hunt.log(line) -- send to Infocyte
        end
    end
    os.remove(temp)
end

tempfile = os.getenv("TEMP") .. "\\CachedKerberosTickets.txt"
file = io.open(tempfile, "r") -- r read mode
if file then
